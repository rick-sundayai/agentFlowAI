...
export async function POST(request: Request) {
  ...
  if (!user) {
    logWithTimestamp(
      "warn",
      "Unauthorized access attempt to Co-Pilot command endpoint"
    );
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const userId = user.id; // Get the authenticated user's ID
  logWithTimestamp("info", "Received Co-Pilot command request for user", {
    userId,
  });
  ...
  try {
    ...
    if (!userCommand) {
      logWithTimestamp("warn", "No command provided in Co-Pilot request", {
        userId,
      });
    ...
    logWithTimestamp("info", "User command received", {
      userId,
      command: userCommand,
    });
    ...
    if (!N8N_COPILOT_WEBHOOK_URL || !N8N_COPILOT_API_KEY) {
      logWithTimestamp(
        "error",
        "Missing N8N Co-Pilot webhook environment variables",
        { userId }
      );
      return NextResponse.json(
        {
    ...
    try {
      ...
      logWithTimestamp("info", "Sending payload to n8n Co-Pilot webhook", {
        userId,
        payload: n8nPayload,
      });
      ...
      logWithTimestamp("info", "Received response from n8n Co-Pilot webhook", {
        userId,
      ...
      if (!n8nResponse.ok) {
        ...
        try {
          ...
          logWithTimestamp(
            "error",
            `n8n Co-Pilot webhook responded with non-OK status ${n8nResponse.status}:`,
            { userId, responseBody: n8nErrorText }
          );
        } catch {
          logWithTimestamp(
            "error",
            `n8n Co-Pilot webhook responded with non-OK status ${n8nResponse.status}, but could not read body.`,
            { userId }
          );
        }
      ...
      if (
        ...
        "response" in n8nResponseBody
      ) {
        logWithTimestamp("info", "Returning n8n response to frontend", {
          userId,
          response: n8nResponseBody.response,
        });
        return NextResponse.json(n8nResponseBody, { status: 200 });
      } else {
        // Handle unexpected response format from n8n
        logWithTimestamp("error", "n8n returned unexpected response format", {
          userId,
          responseBody: n8nResponseBody,
        });
        return NextResponse.json(
          {
            response: {
      ...
      logWithTimestamp(
        "error",
        "Error sending data to n8n Co-Pilot webhook or receiving response:",
        { userId, error: webhookError }
      );
      return NextResponse.json(
        {
          response: {
    ...
    logWithTimestamp("error", "API Route Error processing initial request:", {
      userId,
      error: error,
    });
