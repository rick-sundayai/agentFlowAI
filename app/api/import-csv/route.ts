...
export async function POST(request: Request) {
  ...
  if (!user) {
    logWithTimestamp(
      "warn",
      "Unauthorized access attempt to import-csv endpoint"
    );
  ...
  try {
    ...
    if (!file) {
      logWithTimestamp("warn", "No file uploaded in import-csv request", {
        userId,
      });
      return NextResponse.json({ error: "No file uploaded" }, { status: 400 });
    }

    logWithTimestamp("info", "File received", {
      userId,
      fileName: file.name,
      fileSize: file.size,
    });
    ...
    if (!N8N_IMPORT_WEBHOOK_URL || !N8N_IMPORT_API_KEY) {
      logWithTimestamp(
        "error",
        "Missing N8N import webhook environment variables",
        { userId }
      );
    ...
    try {
      ...
      logWithTimestamp("info", "Sending payload to n8n import webhook", {
        userId,
        fileName: file.name,
      });
      ...
      logWithTimestamp("info", "Received response from n8n import webhook", {
        userId,
        fileName: file.name,
      ...
      if (!n8nResponse.ok) {
        ...
        try {
          ...
          logWithTimestamp(
            "error",
            `n8n import webhook responded with non-OK status ${n8nResponse.status}:`,
            { userId, fileName: file.name, responseBody: n8nErrorText }
          );
        } catch {
          logWithTimestamp(
            "error",
            `n8n import webhook responded with non-OK status ${n8nResponse.status}, but could not read body.`,
      ...
      const successMessage =
        "CSV file received and sent for processing. Your contacts will appear shortly.";
      ...
      logWithTimestamp("info", "Successfully triggered n8n import workflow", {
        userId,
        fileName: file.name,
      });
      ...
      logWithTimestamp(
        "error",
        "Error sending data to n8n import webhook or receiving immediate response:",
        { userId, fileName: file.name, error: webhookError }
      );
      return NextResponse.json(
        {
          error:
            "Failed to connect to the import automation system. Please try again.",
        },
        { status: 500 }
      );
    }
    ...
    logWithTimestamp(
      "error",
      "API Route Error processing initial file upload:",
      { userId, error: error }
    );
